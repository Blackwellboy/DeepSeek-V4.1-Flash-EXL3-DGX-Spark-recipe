...
# POSIX_FADV_DONTNEED — advise the kernel to drop clean page-cache pages for
# a consumed checkpoint range. Used after each safetensors shard is fully
# copied into final model storage on GB10 UMA so reclaimable Cached does not
# starve MemAvailable during long TP4 loads. Opt-out: VLLM_SAFETENSORS_FADV=0.
_POSIX_FADV_DONTNEED = 4


def _fadvise_dontneed_path(path: str) -> None:
    """Range-scoped page-cache drop for a fully consumed checkpoint file.

    Does not use global drop_caches. Best-effort: failures are silent.
    Toggle with VLLM_SAFETENSORS_FADV (default on).
    """
    if os.environ.get("VLLM_SAFETENSORS_FADV", "1") == "0":
        return
    try:
    ...
    try:
        ...
        if size <= 0:
            return
        # Prefer libc posix_fadvise when available; fall back to no-op.
        try:
            libc = ctypes.CDLL("libc.so.6", use_errno=True)
            # posix_fadvise(fd, offset, len, advice) — advise whole file.
            ...
            if rc != 0 and os.environ.get("VLLM_SAFETENSORS_FADV_LOG", "0") == "1":
                logger.debug("posix_fadvise DONTNEED rc=%s path=%s", rc, path)
        ...
        if safetensors_load_strategy == "eager":
            ...
            for name, param in state_dict.items():
                if not should_skip_weight(name, local_expert_ids):
                    yield name, param
            del state_dict
            _adviseadvise_dontneed_path(st_file)
            ...
            # Keep host-side safetensors reads (device=cpu). Direct CUDA
            # safe_open was tried on boot6 and aborted earlier/harder
            # (MemAvailable≈14 GiB) because the CUDA allocator reserved
            # aggressively while file cache was still hot.
            with safe_open(st_file, framework="pt") as f:
                for name in f.keys():  # noqa: SIM118
                    if should_skip_weight(name, local_expert_ids):
                        continue
                    # Disk-backed Engram: never materialize the full embed
                    # tables (~91 GiB class) when disk_backed mode is on.
                    try:
                        ...
                        if should_skip_engram_embed_tensor(name):
                            logger.info_once(
                                "Skipping full Engram embed tensor materialization "
                                "for disk-backed mode: %s",
                                name,
                            )
                            continue
                    except Exception:  # noqa: BLE001
