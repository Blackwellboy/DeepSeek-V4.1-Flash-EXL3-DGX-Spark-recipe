...
class EngramConfig:
    ...
    On DGX Spark / GB10 unified memory, cpu_offload does NOT free physical
    capacity — CPU and GPU share one pool. Prefer disk_backed there."""

    disk_backed: bool = False
    """Store embedding tables on node-local NVMe and stage only looked-up
    rows. Disables full-table Parameter allocation and skips safetensors
    get_tensor materialization for embed.weight/scale. Env override:
