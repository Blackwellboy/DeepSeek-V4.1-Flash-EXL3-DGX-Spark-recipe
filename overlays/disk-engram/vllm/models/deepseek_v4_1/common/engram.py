...
def _engram_head_shard_weight_loader(
    ...
    if getattr(param, "engram_disk_backed", False):
        # Disk-backed tables are bound from safetensors slices; never copy
        # the full shard into a resident Parameter.
        return
...
class ParallelEngramEmbedding(nn.Module):
    ...
    def __init__(
        ...
        if disk_backed and cpu_offload:
            # Disk path is the capacity solution; ignore pinned UVA residency.
            cpu_offload = False
        ...
        self._disk: DiskBackedEngramTable | None = None
        ...
        if disk_backed:
            # Placeholder Parameters (no full-table residency). Weight loader
            # is a no-op; safetensors iterator skips embed.weight/scale.
            ...
            self.weight_scale_inv = nn.Parameter(
                torch.empty(
                    0, dim // block_size, dtype=torch.uint8, device="cpu"
                ),
                requires_grad=False,
            )
            for param in (self.weight, self.weight_scale_inv):
                set_weight_attrs(
                    param,
                    {
                        ...
                        "engram_disk_backed": True,
                    },
                )
            if layer_id is None:
                raise ValueError("disk_backed Engram requires layer_id")
            if max_stage_rows is None:
                try:
                    max_tokens = (
                    ...
                    max_tokens = 8192
                max_stage_rows = max(
                    1024, int(max_tokens) * max(1, dp_size) * self.part_n_hash_cols
                )
            self._disk = DiskBackedEngramTable(
                layer_id=layer_id,
            ...
            logger.info(
                "Engram table DISK-BACKED (non-resident): layer=%s "
                "owned_rows=%d dim=%d stage_rows=%d — FULL_ENGRAM_RESIDENT=NO",
                layer_id,
    ...
    def _storage(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        if self.disk_backed:
            raise RuntimeError("disk-backed Engram has no resident _storage()")
    ...
    def lookup(
        ...
        if not rows:
            return
        if self.disk_backed:
            self._lookup_disk(indices, out, background=background)
            return
    ...
    def _lookup_disk(
        ...
        """Stage only required owned rows from NVMe, then run dequant kernel."""
        assert self._disk is not None
        # Mirror kernel ownership: local = global_index - vocab_start for owned
        # heads. Build a dense [T, local_heads] local-id tensor (or -1).
        t = indices.shape[0]
        local_heads = self.part_n_hash_cols
        head = torch.arange(
            self.head_start,
            self.head_start + local_heads,
            device=indices.device,
            dtype=torch.long,
        )
        # indices columns are global hash-head columns 0..n_hash_cols
        owned = head < self.n_hash_cols
        # Gather global row ids for owned heads; invalid -> -1
        cols = head.clamp(max=self.n_hash_cols - 1)
        gids = indices.index_select(1, cols)  # [T, local_heads]
        valid = owned.view(1, -1) & (gids >= self.vocab_start_idx) & (
            gids < self.vocab_end_idx
        )
        local_ids = torch.where(
            valid, gids - self.vocab_start_idx, torch.full_like(gids, -1)
        )
        weight, scales, inverse = self._disk.fetch_unique_local_rows(local_ids)
        u = weight.shape[0]
        if u == 0:
            out.zero_()
            return
        # Remap local_ids (-1 or row) -> staged row index for kernel with
        # vocab_start=0, vocab_end=U. Build remapped indices [T, n_hash_cols]
        # where only owned heads get staged ids; others stay out of range.
        remapped = indices.clone()
        # inverse aligns with flattened local_ids
        inv = inverse.view(t, local_heads)
...
class Engram(nn.Module):
    ...
    def __init__(
        ...
        disk_backed = engram_disk_backed_enabled(engram_config)
        cpu_offload = (
            False
            if disk_backed
            else (engram_config.cpu_offload if engram_config else True)
        )
